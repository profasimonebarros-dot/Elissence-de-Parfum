...
export default function Portal() {
  ...
  const handleSubmit = async () => {
    ...
    const handlePayWithInfinitePay = async () => {
    if (cart.length === 0) {
      toast({ title: 'O pedido esta vazio', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      const linkRes = await fetch(`${apiBase()}/api/portal/${token}/orders/${order.id}/infinitepay-link`, {
        method: 'POST',
      });
...
const cartPanelBody = (
    <>
      ...
      <div className="p-5 border-t border-border bg-muted/10">
        ...
        {(storeSettings?.infinitepayEnabled ?? false) && (
          <Button
            variant="outline"
            className="w-full h-12 text-base font-medium mt-2"
            disabled={cart.length === 0 || submitting}
            onClick={handlePayWithInfinitePay}
          >
            <CreditCard className="mr-2 h-5 w-5" /> Pagar Online (Cartao/Pix)
          </Button>
        )}
      </div>
    </>
